...
- **Middleware 500**, który **nie wycieka stack trace** użytkownikowi (→ [09](09-prawo-i-ochrona-tworcy.md)).
- **Defensywa na danych z brzegu**: konto OAuth bez hasła, pole `null` tam gdzie kod zakłada string —
  to realne wejścia, gdy wpuścisz prawdziwych userów (często ujawnia się **po swapie bazy**, → [05](05-git-i-wdrozenia.md)).

> Reguła: *throw w jednym żądaniu degraduje to żądanie, nie serwis.* Otestuj regresyjnie (→ [03](03-testowanie-i-weryfikacja.md)).
...
- Idempotencja całości (→ [04](04-skrypty-i-bazy-danych.md)): wznowiony job nie dubluje już zapisanych danych.
...
- **Deliverability poczty to nie „wysłałem".** Zweryfikowana domena nadawcy (SPF/DKIM), realne
  wysłanie testowe, opt-in zgodny z RODO (→ [09](09-prawo-i-ochrona-tworcy.md)). E-mail, który
  „wyszedł", a wylądował w spamie/nigdzie, to bug.
- **Sprawdź to na prodzie/stagingu providera**, nie lokalnie — lokalna sieć nie ma tych blokad (→ [03](03-testowanie-i-weryfikacja.md)).
...
- **Rate-limit + nagłówki bezpieczeństwa** (helmet/limiter) jako stały element stacku (→ [08](08-stack-i-technologie.md)).
- Powiąż z prawem i regulaminem: limit i jego komunikacja to też ochrona przed abuse (→ [09](09-prawo-i-ochrona-tworcy.md)).
...
- 🚫 **Sesje w pamięci procesu** — każdy deploy wylogowuje wszystkich (→ [05](05-git-i-wdrozenia.md)).

## Dla nowych projektów
Wpisz do Dnia 0 (→ [07](07-nowy-projekt-checklist.md)) **zanim** pojawi się pierwszy realny user:
...
Każdy incydent prod → wpis w runbooku z datą i ponumerowaną lekcją (→ [05](05-git-i-wdrozenia.md), [06](06-wspolpraca-i-pamiec.md)).
