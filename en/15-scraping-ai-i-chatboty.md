...
> backoff, rotation, quotas, resumability) lives in → [14](14-odpornosc-operacyjna.md); this chapter is
...
- **Respect the source.** Honor `robots.txt`/ToS, throttle reasonably, identify yourself. The goal is
  **data, not a DoS** — an aggressive scraper is a legal and ethical problem (→ [09](09-prawo-i-ochrona-tworcy.md)).
- **Parse defensively.** Selectors break on redesigns — **validate the shape** of the result, not just the
  status (the classic "200 + error page," → [14](14-odpornosc-operacyjna.md)). Selector drift → an alert, not silent zeros.
- **Normalize at the entry point.** Web data is dirty: fuzzy-match and dedup against **lookup tables**,
  slug, one source of truth (→ [11](11-model-danych-normalizacja.md)). Raw material goes in, a clean record stays.
- **Incrementally.** Fetch the **delta** (what changed since last time), not the whole catalog every night —
  cheaper, faster, lighter on the source. Checkpoint and resumability → [14](14-odpornosc-operacyjna.md).
- **Idempotent loaders.** Re-importing the same data creates no duplicates (→ [04](04-skrypty-i-bazy-danych.md)).
...
- **Match the model to the task.** Cheap/fast for the simple things (classification, extraction), powerful
  for complex reasoning. Claude as the default (→ [08](08-stack-i-technologie.md)). **Measure cost and latency**, don't guess.
...
- **Cache + idempotency.** The same inputs → a stored result; don't call the LLM in a loop over what hasn't
  changed. Expensive calls are cost and latency (→ [13](13-wydajnosc-frontend-i-sql.md)).
- **Quotas and budget.** A limit per user/endpoint, cost monitoring, a hard quota on a paid API (→ [14](14-odpornosc-operacyjna.md)).
- **Batch when you don't need realtime.** Enriching data offline in a pipeline with checkpoints (→ [14](14-odpornosc-operacyjna.md))
...
- **Grounding is mandatory.** The assistant answers from **your data** (context from the database / RAG),
  not the model's memory. Cite the source (→ [11](11-model-danych-normalizacja.md)). A hallucinated price/fact is a bug, not an "AI feature."
...
- **Streaming UX.** A token-by-token response (SSE), conversation history, clear limits (→ [13](13-wydajnosc-frontend-i-sql.md), [14](14-odpornosc-operacyjna.md)).
- **Security (prompt injection).** User content and web data are **data, not instructions**. Don't perform
  side-effect actions from the chat without confirmation, and don't leak secrets or the system prompt (→ [09](09-prawo-i-ochrona-tworcy.md)).
- **Eval like tests.** A golden set of control questions; a prompt change → run it through and measure
  regressions (→ [03](03-testowanie-i-weryfikacja.md)). A prompt with no eval drifts silently.
- **Law.** A disclaimer ("this is not professional advice"), GDPR for storing conversations and data (→ [09](09-prawo-i-ochrona-tworcy.md)).
...
- 🚫 **No limit/quota** — chat is an open bill and an abuse vector (→ [14](14-odpornosc-operacyjna.md)).
...
and grounded** from the start — the system prompt as data, a golden set as a test (→ [03](03-testowanie-i-weryfikacja.md)),
a limit as a rule (→ [14](14-odpornosc-operacyjna.md)). The rest grows with the project.
