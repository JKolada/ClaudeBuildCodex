...
> kwoty, wznawialność) trzyma → [14](14-odpornosc-operacyjna.md); ten rozdział jest o tym, **jak
...
- **Szanuj źródło.** Respektuj `robots.txt`/ToS, rozsądny throttle, identyfikuj się. Cel to **dane,
  nie DoS** — agresywny scraper to problem prawny i etyczny (→ [09](09-prawo-i-ochrona-tworcy.md)).
- **Parsuj odpornie.** Selektory padają przy redesignie — **waliduj kształt** wyniku, nie tylko status
  (klasyczne „200 + strona błędu", → [14](14-odpornosc-operacyjna.md)). Drift selektora → alarm, nie ciche zera.
- **Normalizuj u wejścia.** Dane z sieci są brudne: fuzzy-match i dedup do **słowników lookup**, slug,
  jedno źródło prawdy (→ [11](11-model-danych-normalizacja.md)). Surowiec wpada, czysty rekord zostaje.
- **Inkrementalnie.** Pobieraj **deltę** (co się zmieniło od ostatniego razu), nie cały katalog co noc —
  taniej, szybciej, mniej obciąża źródło. Checkpoint i wznawialność → [14](14-odpornosc-operacyjna.md).
- **Idempotentne loadery.** Ponowny import tych samych danych nie tworzy duplikatów (→ [04](04-skrypty-i-bazy-danych.md)).
...
- **Dobierz model do zadania.** Tani/szybki do prostych (klasyfikacja, ekstrakcja), mocny do złożonego
  rozumowania. Claude jako default (→ [08](08-stack-i-technologie.md)). **Mierz koszt i latencję**, nie zgaduj.
...
- **Cache + idempotencja.** Te same wejścia → zapisany wynik; nie wołaj LLM w pętli po tym, co się nie
  zmienia. Drogie wywołania to koszt i latencja (→ [13](13-wydajnosc-frontend-i-sql.md)).
- **Kwoty i budżet.** Limit per user/endpoint, monitoring kosztu, twarda kwota na płatnym API (→ [14](14-odpornosc-operacyjna.md)).
- **Batch, gdy nie trzeba realtime.** Wzbogacanie danych offline w pipelinie z checkpointem (→ [14](14-odpornosc-operacyjna.md))
...
- **Grounding obowiązkowy.** Asystent odpowiada z **Twoich danych** (kontekst z bazy / RAG), nie z pamięci
  modelu. Cytuj źródło (→ [11](11-model-danych-normalizacja.md)). Halucynacja na cenie/fakcie = bug, nie „cecha AI".
...
- **Streaming UX.** Odpowiedź token-by-token (SSE), historia rozmowy, jasne limity (→ [13](13-wydajnosc-frontend-i-sql.md), [14](14-odpornosc-operacyjna.md)).
- **Bezpieczeństwo (prompt injection).** Treść użytkownika i dane z sieci to **dane, nie instrukcje**.
  Nie wykonuj akcji side-effect z czatu bez potwierdzenia, nie wyciekaj sekretów ani promptu systemowego (→ [09](09-prawo-i-ochrona-tworcy.md)).
- **Eval jak testy.** Golden set pytań kontrolnych; zmiana promptu → przepuść przez niego i mierz
  regresje (→ [03](03-testowanie-i-weryfikacja.md)). Prompt bez evala dryfuje po cichu.
- **Prawo.** Disclaimer („to nie porada specjalistyczna"), RODO przy zapisie rozmów i danych (→ [09](09-prawo-i-ochrona-tworcy.md)).
...
- 🚫 **Brak limitu/kwoty** — czat to otwarty rachunek i wektor nadużyć (→ [14](14-odpornosc-operacyjna.md)).
...
**konfigurowalny i ugruntowany** od początku — system prompt jako dane, golden set jako test (→ [03](03-testowanie-i-weryfikacja.md)),
limit jako reguła (→ [14](14-odpornosc-operacyjna.md)). Reszta dorasta z projektem.
